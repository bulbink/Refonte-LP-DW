...
class pgm_ResourceLoader {
     ...
     * @return pgm_ResourceLoader
     */
    public static function instance() {
	if (self::$instance == null) {
	    self::$instance = new pgm_ResourceLoader();
	}
	...
	foreach ($bundle as $bundle_entry) {
		...
		$path = pgm_Helper::arr_get($bundle_entry, 0);
		$is_local = pgm_Helper::arr_get($bundle_entry, 1);
	    } else {
		$path = $bundle_entry;
		$is_local = true;
	    }
	...
	$params = $this->prepare_params($path, $is_local);
	if ($params == null) {
	    //echo (pgm_Registry::get('PLUGIN_DEBUG_MODE')) ? "Invalid path {$path}" : '';
	    return; //bad file
	}
	...
	$count = pgm_Helper::arr_get($this->resource_counter, $identifier, 0);
	...
	$name = str_replace(array('/', ':', '_', '.'), '-', pgm_Registry::get('PLUGIN_SLUG') . '-' . $this->identifier($file_path));
	$file = pgm_Registry::get('PLUGIN_DIR') . $this->clean($file_path);
	$url = ($is_local) ? pgm_Registry::get('PLUGIN_URL') . $file_path : $file_path;
	...
	return pgm_Registry::get('PLUGIN_URL') . $file;
    }
